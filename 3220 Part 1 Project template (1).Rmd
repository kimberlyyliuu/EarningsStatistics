...
p2<-ggplot(incomeData, aes(x = H_ED, y = PEARNVAL, color = Education_Level)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.text = element_text(size = 8), # Shrink the text in the legend
        legend.title = element_text(size = 9), # Shrink the legend title
...
```{r, echo=FALSE, fig.width=6, fig.height=8, out.width='0.50\\linewidth', out.height='0.95\\textheight', results="asis"}
plot_grid(p1, p2, ncol = 1, nrow = 2)
plot_grid(p3, p4, ncol = 1, nrow = 2)
```

The histogram titled Distribution of Personal Earnings by State depicts the distribution of the response variable PEARNVAL which represents the average earnings of individuals in each state/territory. The graph is unimodal, mostly symmetrically distributed, and displays minimal right skew. Because of this, personal earnings are appropriate for linear regression analysis. The scatterplot Education Attainment vs Personal Earnings displays the relationship between a state's average education and the average personal earnings of its citizens. The graph has a linear slope with a correlation coefficient of .8055 which suggests a strong relationship between the response and explanatory variable. There is one outlier, Washington DC, whose highest education and personal earnings is significantly higher than the rest. The Age vs Personal Earnings scatterplot shows the relationship between the average age of a state(excluding individuals under the age of 18) and the average personal earnings of the state. The data points seem to not display a pattern and the correlation coefficient of .1031 suggests that their is no linear relationship between the variables. The Health Status vs Personal Earnings scatterplot seems to depict a relatively strong negative linear correlation and a correlation coefficient of -0.764 suggests this to be true. Again, Washington D.C. is an outlier. 
