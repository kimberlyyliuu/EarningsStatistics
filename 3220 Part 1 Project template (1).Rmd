...
firstset<-firstset[(c("PERIDNUM", "PRDTRACE", "A_HGA", "A_SEX", "PTOT_R", "PEARNVAL", "A_MARITL", "HEA", "MIG_DIV", "PERIDNUM", "A_AGE"))]

# Renaming Variables
colnames(firstset) <- c("IDNUM", "RACE", "H_ED", "SEX", "TOT_IN", "PEARNVAL", "MARITL", "HEA", "RES_DIV", "IDNUM_MATCHER", "AGE")
...
#Remove people with PEARN_VAL = 0 
...
finalData <- compiledData |>
  group_by(STATE) |>
  summarize(across(4:11, mean, na.rm = T))
```

Making final dataset available on GitHub: 
```{r}
#Create csv file
write.csv(finalData, "finalData", row.names = F)
#Raw file link
# https://raw.githubusercontent.com/kimberlyyliuu/STAT3220Project/main/finalData
```
...
*Some of the categorical variables are no longer informative when an average is taken: 
-MARITL
-RES_DIV 

Remove from set: 
```{r}
incompleteData<-finalData[,-c(6,8)]
```


*Still need one more quant, so make one of the var that replaces the above two quant.

Potential variables to replace: 
-Sales tax by state
-Percentage unionized by state
-Ratio of nonwhite state residents
-urban population measured as a ratio 
...
UrbanPopData<-UrbanPopData[-52,]

# Merge with set
incompleteData<-merge(incompleteData, UrbanPopData, by.x="STATE")
```
...
completeData<-merge(incompleteData,TaxData,by.x="STATE", by.y="FIPS")
